//<![CDATA[
	...
	function dependancy(extra) {
        ...
        function xhr(){
            ...
            if(!http_request){
                alert('不能创建Ajax对象，请更换浏览器');
            }
            return http_request;
        }
        
        function loadTemplate(name) {
            var http_request = xhr();
            //http_request.setRequestHeader('Content-type','application/x-www-form-urlencoded');
            http_request.onreadystatechange=function()
            {
                if (http_request.readyState==4 && http_request.status==200)
                {
                    document.getElementById('txt').value = http_request.responseText;
                    document.getElementById('preview').style.display='';
                }
            }
            http_request.open("GET",'<?php echo _SITE ?>ajax.php?action=template&name='+name,true);
            http_request.send();
            return true;
        }
        function updateEmpty(id,type){
            var value=prompt("请输入空置面积信息","");
            ...
            http_request.open("GET",'<?php echo _SITE ?>ajax.php?action=update&id='+id+'&type='+type+'&value='+value,true);
            http_request.send();
            return true;
        }
