...
function db($variable) {
	...
	# Edit database connection information only
		'dbhost' => '192.168.1.100', // MySQL host
		'dbname' => 'shuncheng', // Database name
		'dbuname' => 'dev', // Database Username
...
connect_to_db();
	...
	//SEF links of the hardcoded items - RESERVED WORDS - will clash if using for article/category seftitles.
		$l['cat_listSEF'] = 'building,archive,contact,sitemap,login';
...
if(!empty($_POST['submit_text']) && !_ADMIN){
...
function articles() {
	...
	if (!$result || !$numrows) {
		...
		while ($r = mysql_fetch_array($result)) {
			...
			file_include(str_replace('[break]', '',$text), $shorten,$r['aid']);
			...
			$edit_link.= ' '.l('divider').' '.$visiblity;//?action=snews_files&aid=10
            if($subcatSEF=='proxy') $edit_link.= ' . <a href="'._SITE.'?action=snews_files&amp;aid='.$r['aid'].'">图片</a>';
			if (!empty($currentPage)) {
				if ($infoline == true) {
...
function archive($start = 0, $size = 200) {
	...
	if ($count === 0) {
		...
		while ($r = mysql_fetch_array($result)) {
			...
			echo '<p>('.$r['name'].')<a href="'._SITE.$link.'/'.$r['asef'].'/">
				'.$r['title'].' </a></p>';
		}
		echo'</div>';
	}
...
function sitemap() {
	echo '<h2>'.l('sitemap').'</h2>
		<h3><strong>'.l('pages').'</strong></h3><div class="navtit">';
	$link = '<a href="'._SITE;
	echo $link.'">'.l('home').'</a>';
	echo $link.'archive/">'.l('archive').'</a>';
	// Patch #16 - 1.7.1
	...
	$result = mysql_query($query);
	while ($r = mysql_fetch_array($result)) {
		echo $link.$r['seftitle'].'/">'.$r['title'].'</a>';
	}
	echo $link.'contact/">'.l('contact').'</a>';
	echo $link.'sitemap/">'.l('sitemap').'</a>';
	echo '</div>
...
function search($limit = 20) {
	...
	if (strlen($search_query) < 4 || $search_query == l('search_keywords')) {
		...
		if (!$numrows) {
			...
			while ($r = mysql_fetch_array($result)) {
				$date = date(s('date_format'), strtotime($r['date']));
				# Patch #20 - 1.7.1 - next string patched
...
function form_articles($contents) {
	...
	$catnum = mysql_fetch_assoc(mysql_query("SELECT COUNT(id) as catnum FROM "._PRE.'categories'.""));
 	if ($contents == 'article_new' && $catnum['catnum'] < 1) {
        ...
        echo '<script type="text/javascript" src="/kindeditor/kindeditor-min.js"></script>';
        echo '<script type="text/javascript" src="/kindeditor/lang/zh_CN.js"></script>';
        echo "<script type=\"text/javascript\">
var editor;
			KindEditor.ready(function(K) {
				editor = K.create('textarea[id=\"txt\"]', {
					resizeType : 1,
		...
		echo html_input('textarea', 'text', 'txt', $frm_text, l('text'), '', '', '', '', '', '8', '100', '', '', '');
		buttons();
...
function files() {
 	...
 	if ($ip && $time && $upload_file && _ADMIN) {
		...
		if ($_FILES['imagefile']['type']) {
            ...
            $filename = time().strtolower(substr($filename,strripos($filename, '.')));
			if (!in_array(substr(strrchr($filename, '.'), 1), $extension) || in_array($filename, $ignore)) {
                ...
                if(empty($_POST['aid']))
                    ...
                    echo notification(0,'',array('action=snews_files','aid='.$_POST['aid']));
		...
		if (isset($_GET['task']) == 'delete') {
			...
			if(empty($_GET['aid']))
                    ...
                    echo notification(0,'',array('action=snews_files','aid='.$_GET['aid']));
		} else {
            $target_dir = '.';
            if(empty($_POST['upload_dir']) && !empty($_GET['aid'])){
                $dir = './caseimg/'.(int)$_GET['aid'];
                $_POST['upload_dir'] = $dir;
                $_POST['view_dir'] = $dir;
            }
			echo '<div class="adminpanel">';
            ...
            echo html_input('hidden', 'aid', 'aid1', empty($_GET['aid'])?'0':$_GET['aid'], '', '', '', '', '', '', '', '', '', '', '');
		...
		if(!in_array($file, $ignore)) {
			...
			switch(true) {
				case ($mode == 'option' && is_dir($target)):
					$selected = $_POST['view_dir'] == $target ? ' selected' : '';
			  	...
			  	case ($mode == 'list' && is_file($target) && in_array(substr(strrchr($target, '.'), 1), $types)):
                    $link = '?action=snews_files&amp;task=delete&amp;folder='.$path.'&amp;file='.$file;
                    if(!empty($_GET['aid'])) $link .= '&amp;aid='.$_GET['aid'];
			  		echo '
					<a href="'.$target.'" title="'.l('view').' '.$file.'">'.$file.'</a>
						'.l('divider').'
					<a href="'.$link.'" title="'.l('delete').' '.$file.'" onclick="return pop()">	'.l('delete').'</a><br />'; // Patch #3 - 1.7.0
			  		break;
			}
		}
	}
...
function connect_to_db() {
    ...
    mysql_query('set names '.db('charset'));//Patch #* By skding
}
...
function notification($error = 0, $note = '', $link = '') {
	...
	switch(true){
        case (is_array($link)):
            $goto = '<p><a href="'._SITE.'?'.implode('&', $link).'" title="'.l('back').'">'.l('back').'</a></p>';
            break;
		case (!$link):
			$goto = '';
			break;
...
//FILE INCLUSION
function file_include($text, $shorten, $aid=0) {
   ...
   if(substr_count ($fulltext, '&')>0){$fulltext = str_replace('&', '&amp;', str_replace('&amp;', '&', $fulltext));}
    ...
    if ($ins > 0) {
		...
		while ($i <= $num) {
			$func = explode(':|:', $text[$i]);
			ob_start();
            if($func[0]=='dispalyCaseImgs')
                $returned = call_user_func_array($func[0], $aid);
            else
