...
MT.prototype.pop={
    alert:function(a,c){
        ...
        if(b.hasBtn_ok)d+=b.hasBtn_cancel?'<div class="btn"><a class="W_btn_b MT_btn_callBack" href="javascript:void(0)"><span>'+b.ok_label+'</span></a><a class="W_btn_a MT_btn_cancel" href="javascript:void(0)"><span>'+b.cancel_label+"</span></a></div>":'<div class="btn"><a class="W_btn_b MT_btn_callBack" href="javascript:void(0)"><span>'+b.ok_label+"</span></a></div>";
...
$.jqDnR={
    dnr:{},e:0,ed:function(){},drag:function(a){if(M.k=="d"){var c=M.X+a.pageX-M.pX;a=M.Y+a.pageY-M.pY;if(c<0)c=0;else if(c>=B.W-M.W)c=B.W-M.W-10;if(a<0)a=0;else if(a>B.H+$(window).scrollTop()-M.H)a=B.H+$(window).scrollTop()-M.H;E.css({left:c,top:a})}else E.css({width:Math.max(a.pageX-M.pX+M.W,0),height:Math.max(a.pageY-M.pY+M.H,0)});return false},stop:function(){ED(dt);dt.css({top:E.offset().top,left:E.offset().left});E.remove();$().unbind("mousemove",J.drag).unbind("mouseup",J.stop)}
};
    ...
    return a.each(
        function(){
            ...
            c.bind("mousedown",{e:a,k:b},
                function(h){
                    var g=h.data,k={};dt=g.e;d(dt);E&&E.remove();E=$('<div id="drag_div" style="cursor:move"></div>').appendTo(document.body).css({position:"absolute",top:dt.offset().top+"px",left:dt.offset().left+"px",width:dt.width()+"px",height:dt.height()+"px",border:"#DDD 3px solid"});
                    if(E.css("position")!="relative")try{E.position(k)}catch(l){}M={X:k.left||f("left")||0,Y:k.top||f("top")||0,W:f("width")||E[0].scrollWidth||0,H:f("height")||E[0].scrollHeight||0,pX:h.pageX,pY:h.pageY,k:g.k,o:E.css("opacity")};E.css({opacity:1});$().mousemove($.jqDnR.drag).mouseup($.jqDnR.stop);return false
                }
            )
        }
    )
