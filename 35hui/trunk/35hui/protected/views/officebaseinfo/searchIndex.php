...
<div class="search">
    <div class="sctit">
        <form method="post" id="headSearchForm" action="<?php echo Yii::app()->createUrl('site/officeheadsearch');?>" onsubmit="changeValue('submit')">
            ...
            ?>
            <input type="hidden" name="type" id="form_type" value="1" />
    ...
    <div class="scleft">
        <div class="schtit">
            ...
            <div class="schtrt">
                <?php
                if($saleOrRent==1){
                    echo CHtml::link("切换到经纪人找房模式",array("/uagent/officerent"),array("class"=>"listbg"));
                }else{
                    echo CHtml::link("切换到经纪人找房模式",array("/uagent/officesale"),array("class"=>"listbg"));
                }
                ?>
            </div>
            <div class="schtpage">
        ...
        if($allSource){
            foreach($allSource as $data){
                if($saleOrRent==1){
                    ...
                    $this->renderPartial('_saleview', array('data'=>$data));
                }
            }
        }else{
    ...
    $(document).ready(function(){
        ...
        $(".schcont .gemore span").bind("click", function(){
            ...
            if(typeof(build)=="undefined"){
                ...
                $.ajax({
                    url:"/officebaseinfo/ajaxgetsource",
                    type:"GET",
                    data:{"start":start,"saleOrRent":"<?=$saleOrRent?>","buildid":build,"condition":condition},
                    success:function (msg){
                        $(table).find("tr").last().remove();
                        $(obj).next("span").css("display","block");
                        var msg = eval(msg);
                        var danwei = "<?=$saleOrRent==1?"元/平米·天":"万元/套"?>";
        ...
        var cookieName = "<?=$saleOrRent==1?"officerent":"officesale"?>";
        var danwei = "<?=$saleOrRent==1?"元/平米•天":"元/平米"?>";
