...
if($buildinfo){
    //获取租售信息
    $all = Officebaseinfo::model()->getSaleOrRentSourceByCondition($buildinfo->sbi_buildingid, 1, @$_GET["search"]);
    ?>
<div class="schcont">
    <div class="schdes" style=" background: none;">
        <div class="schpic"><a href="<?=Yii::app()->createUrl("systembuildinginfo/view",array("id"=>$data->ob_sysid,"srtp"=>"1"))?>" target="_blank"><img src="<?=Picture::model()->getPicByTitleInt($buildinfo->sbi_titlepic,"_normal");?>" /></a></div>
        <div class="schtxt">
            <h2><a href="<?=Yii::app()->createUrl("systembuildinginfo/view",array("id"=>$data->ob_sysid,"srtp"=>"1"))?>" target="_blank"><?=$buildinfo->sbi_buildingname?></a></h2>
            ...
            <p>开发商：<?=$buildinfo->sbi_developer?></p>
        </div>
        <div class="schpk" style="position: relative;">
            <div style=" position: absolute;left:-58px; width: 190px;">
                <div class="pkt"><?=$buildinfo->sbi_avgrentprice?"<code style='color:#808080;'>参考租金：</code><em>".$buildinfo->sbi_avgrentprice."</em>元/平米•天":""?></div>
            <div class="pkb" style="padding-top:23px;"><input type="checkbox" id="build_<?=$data->ob_sysid?>" value="<?=$data->ob_sysid?>" attrprice="<?=$buildinfo->sbi_avgrentprice?$buildinfo->sbi_avgrentprice:"0"?>" attrname="<?=common::strCut($buildinfo->sbi_buildingname,24)?>"/> <label for="build_<?=$data->ob_sysid?>">加入比较</label></div>
        </div></div>

        <div class="schtan" style="display:none">
            <div class="schbord">
                <div class="rtcont" style="display:none">
                    <a href="<?=Yii::app()->createUrl("systembuildinginfo/view",array("id"=>$data->ob_sysid,"srtp"=>"1"))?>" target="_blank"><img src="<?=Picture::model()->getOnePicExceptTitleInt($data->ob_sysid,1,$buildinfo->sbi_titlepic,"_large");?>" width="286px" height="200px"/></a>
                   <div class="tb_up">
    ...
    <div class="tabcont">
        <table border="0" cellpadding="0" cellspacing="0" class="table_01">
            ...
            <tr>
                ...
                <td class="txt"><?=$value->ob_rentprice."元/平米·天"?></td>
