<div class="schcont">
    <div class="zerocontent">
        <?php
        if(isset($options["keyword"])){//如果有关键词搜索.要查看是否存在这个楼盘。
            $model = Systembuildinginfo::model()->findByAttributes(array("sbi_buildingname"=>urldecode($options["keyword"])));
            if($model){//表明有楼盘，却没发布过房源
                ?>
        <div class="sorry_title">抱歉，您查找的楼盘暂时没有房源！点击浏览楼盘概况！</div>
        <ul class="noresult">
        ...
        <ul class="noresult">
            ...
            <li>·<a href="<?=Yii::app()->createUrl("systembuildinginfo/buildlist",SearchMenu::dealOptions($options))?>" target="_blank">去楼盘字典处浏览全楼盘信息</a></li>
