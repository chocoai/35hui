...
<div class="lof-slidecontent" id="lofslidecontent45" >
        ...
        <ul class="lof-main-wapper lof-opacity">
			...
			if($data){
				foreach($data as $val){?>
					...
					<a target="_blank" href="<?php echo $this->createUrl('systembuildinginfo/view',array('id'=>$val->p_sourceid,'tag'=>'album')) ?>">
						<img class="img_SN" src="<?=PIC_URL.Picture::showStandPic($val->p_img,"")?>" alt="<?=$val->p_title?>" title="<?=$data->p_title?>">
   ...
   <div class="lof-navigator-wapper">
        ...
        <div class="lof-navigator-outer">
			...
			if($data){
				foreach($data as $val){?>
					<li><img src="<?=PIC_URL.Picture::showStandPic($val->p_tinyimg,"_large")?>" alt="<?=$val->p_title?>"></li>
			<?	}	
			}
			?>                      
            </ul>
