...
<div class="search">
    <div class="sctit">
        <form method="post" id="headSearchForm" action="<?php echo Yii::app()->createUrl('site/shopheadsearch');?>" onsubmit="changeValue('submit')">
            ...
            <input type="hidden" name="type" id="form_type" value="<?=($type+2)?>" />
            <input type="hidden" name="sellorrent" id="sellorrent" value="1" />
    ...
    <div class="scleft">
        <div class="schtit">
            ...
            <div class="schtrt">
                <?php
                if($type==1){
                    echo CHtml::link("切换到经典找房模式",array("/shop/rentindex"),array("class"=>"listbg"));
                }
                if($type==2){
                    echo CHtml::link("切换到经典找房模式",array("/shop/sellindex"),array("class"=>"listbg"));
                }
                if($type==3){
                    echo CHtml::link("切换到经典找房模式",array("/shop/transferindex"),array("class"=>"listbg"));
                }
                ?>
            </div>
            <div class="schtpage">
        ...
        if($allSource){
            foreach($allSource as $data){
                $this->renderPartial('_shopsearchindex', array('data'=>$data,'onlineUser'=>$onlineUser,'type'=>$type));
            }
        }else{
        ?>
        <div class="schcont">
    ...
    $(document).ready(function(){
        ...
        $(".schcont .gemore span").bind("click", function(){
            ...
            if(typeof(userId)=="undefined"){
                ...
                if($(table).find("tr").length<=4){//点击收起按钮
                ...
                $.ajax({
                    url:"/uagent/ajaxgetshop",
                    type:"GET",
                    data:{"start":start,"userId":userId,"condition":condition,'type':type},
                    success:function (msg){
                        ...
                        for(var i=0;i<msg.length;i++){
                            ...
                            html +='<td class="txt">'+msg[i].officearea+'㎡</td>';
