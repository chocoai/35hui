...
$num=Oprationconfig::model()->getConfigByName('ua_practice_audit');
?>

<?php $this->renderPartial('_head'); ?>

<div class="msg">
    通过上传名片扫描件，以提升用户的资质，正确绑定后奖励<?=$num[1]?>点积分。如果提供了身份认证或名片认证， 每类房源可发布<?=$unAllCertificate[0]?>条,每天可刷新<?=$unAllCertificate[2]?>次,每类房源可录入总数为<?=$unAllCertificate[1]?>条，如果两种认证都提供了，对应的数量为：<?=$AllCertificate[0]?>，<?=$AllCertificate[2]?>，<?=$AllCertificate[1]?>。
</div>
<div class="certmain">
    <div class="certlf"style="border:none;">
        ...
        <table class="table_01" style="width:300px">
            <?php
            if($uagent->ua_bcardurl!=""&&$uagent->ua_bcardaudit==1){//已经上传，并且审核通过
                $time = date('Y-m-d H:i',$uagent->ua_bcardtime);
                ?>
            <tr>
                <td>
                    <img src="<?=Picture::showStandPic(PIC_URL.$uagent->ua_bcardurl,"_large"); ?>" style="width:297px;height:210px;" />
                </td>
            </tr>
            <tr>
                <td><?="已认证于 ".$time; ?></td>
                ...
                <?php
            }else if($uagent->ua_bcardurl!=""&&$uagent->ua_bcardaudit=="2"){//已上传，审核未通过
                ?>
            <tr>
                <td><img src="<?=Picture::showStandPic(PIC_URL.$uagent->ua_bcardurl,"_large"); ?>" style="width:297px;height:210px;" /></td>
            </tr>
            <tr>
            ...
            <tr id="submitButton">
                ...
                <?php
            }else if($uagent->ua_bcardurl!=""&&$uagent->ua_bcardaudit=="0"){//未审核
                ?>
            <tr>
                <td><img src="<?=Picture::showStandPic(PIC_URL.$uagent->ua_bcardurl,"_large"); ?>" style="width:297px;height:210px;" /></td>
            </tr>
    ...
    <div class="certlf" >
        <p><strong class="red">合格名片案例：</strong></p>
        <p>合格名片要求</p>
        <p>1、确保名片上的内容清晰可见</p>
        <p>2、确保手机号码和注册时的手机号码一致</p>
        <p>3、文件格式仅限jpg、gif、png文件，小于2M</p>
        <div class="certpic"><img src="/images/mc/hege.png" /></div>
    </div>
</div>
