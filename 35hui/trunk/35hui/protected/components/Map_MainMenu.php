 ...
 */
class Map_MainMenu extends CWidget
{
	public $items=array();
	...
	public function run()
	{
		...
		$this->render('Map_mainMenu',array('items'=>$items));
	}
	...
	protected function isActive($pattern,$controllerID,$actionID)
	{
		...
		if(strpos($pattern[0],'/')!==false){
            $matched=strtolower($pattern[0])===strtolower($controllerID.'/'.$actionID);
        }else{
			$matched=strtolower($pattern[0])===strtolower($controllerID);
        }
        return $matched;
	}
}