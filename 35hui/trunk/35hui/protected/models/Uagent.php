...
class Uagent extends CActiveRecord
{
    ...
    public function getRecentUpdate($userId, $limit=10){
        ...
        $criteria->limit = $limit;
        $recentShop = Shopbaseinfo::model()->findAll($criteria);
    ...
    public function getMangeUserById($id){
        
        $model = self::findByAttributes(array("ua_uid"=>$id));
        if($model->ua_muid){
        $mangeuser=Manageuser::model()->findByPk($model->ua_muid);
            if($mangeuser){
               $tel= $mangeuser['mag_tel']?$mangeuser['mag_tel']:"";
               $mangeusername=$mangeuser['mag_realname']?$mangeuser['mag_realname']:"";
               return $mangeusername."&nbsp;&nbsp;&nbsp;&nbsp;".$tel;
            }
        }
        return "";
    }
