...
class User extends CActiveRecord
{
    ...
    */
     ...
     * 级别名称
     */
    public static $titleArr = array('置业助理(离线)','高级经纪(离线)','资深经纪(离线)','星级主任(离线)');
    /*
     * 级别分段
     */
...
//        );
        $img = "lv1.png";
        $i = "-1";
        foreach(self::$pointArr as $key=>$value){
            $i +=1;
            if($point>=$key){
                $img = $value;
                $title =self::$titleArr[$i];
            }
        }
     ...
     * 获取用户的图片链接对象
     */
    public function getUserShowPic($id,$link=true) {
        $user = $this->findbyAttributes(array('user_id'=>$id));
        $userName=$userLink='';
        if($user) {
        ...
        $userPic=CHtml::image($this->getUserHeadPic($id,"_large"),$userName,array('height'=>'65px','width'=>'50px'));
        if(!$link)
            return $userPic;
        if($userLink)
            return '<a target="_blank" href="'.$userLink.'">'.$userPic.'</a>';
        return $userPic;
    }
    /**
     * 获取用户的真实名称
