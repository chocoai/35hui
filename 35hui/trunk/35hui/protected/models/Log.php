...
class Log extends CActiveRecord
{
    ...
    public static $moneyTemplate = array(
        "1"=>"今日第一次登录，系统赠送{:money}新币",
        "2"=>"申请拍摄全景成功，扣除{:money}新币",
        "3"=>"购买广告栏位成功，扣除{:money}新币",
        "4"=>"修改房源成功，扣除{:money}新币"
    );
    /**
     *积分操作日志模板
     * @var <array>
...
//        if(!$num) return false;
    ...
    /**
     *根据类型，得到当前登录用户的几条log记录
     * @param <type> $lg_type 类型，积分还是新币
     * @param <type> $limit条数
     * @return <type>
     */
    public function getCurrentUserLogsByType($lg_type,$limit=5) {
        $userId = Yii::app()->user->id;
        $criteria=new CDbCriteria;
        $criteria->addColumnCondition(array("lg_type"=>$lg_type,"lg_userid"=>$userId));
        $criteria->limit = $limit;
        $criteria->order = "lg_recodetime desc";
        $logs = $this->findAll($criteria);
        return $logs;
    }
}