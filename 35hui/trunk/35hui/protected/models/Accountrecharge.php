...
class Accountrecharge extends CActiveRecord
{
	...
	public function search()
	{
		...
		$criteria->compare('arc_ordernum',$this->arc_ordernum);
...
//        $total_fee    = $pirce;
    ...
    public function oprateOrderByOrderNum($orderNum, $alipaynum){
        ...
        $fundsconfigModel = $model->fundsconfig;
        //执行赠送物品。
        Userproperty::model()->addMoney($model->arc_uid, $fundsconfigModel->fc_giveprice, "充值".$fundsconfigModel->fc_rmbprice."元，兑换{:money}新币");
        Userproperty::model()->addPoint($model->arc_uid, $fundsconfigModel->fc_givepoint, "充值".$fundsconfigModel->fc_rmbprice."元，兑换{:point}积分");
        //User::model()->setUserVip($model->arc_uid,$model->arc_fcid);
        return true;
    }
    /**
     * 得到用户最新一次成功购买套餐的时间，
     * @param integer $user 用户id，为空取当前登录用户id
     * @return string
     */
    public function getUserNewlyVip($user=0){
        if(!$user)
            $user=Yii::app()->user->id;
        $mod=$this->find(array('condition'=>'arc_state=1 AND arc_uid='.$user,'order'=>'arc_rechargetime DESC'));
        if($mod && $mod->arc_rechargetime)
            return $mod->arc_rechargetime;
        return '0';
    }
}