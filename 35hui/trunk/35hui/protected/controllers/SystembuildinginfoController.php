	...
	*/
    ...
    public function viewComment(){
        ...
        $this->render('systembuildingcomment',array(
            "model"=>$model,
            'comments'=>self::getComment(1,$id,"10",$order),
            'ifShop'=>false,
        ));
    }
    /**
...
//        }
        ...
        $srtp ='1';//1rent,2sell
        if(isset($_GET["srtp"])&&$_GET["srtp"]){
            if($_GET["srtp"]==1||$_GET["srtp"]==2)
            $srtp =$_GET["srtp"];
        }
...
//            'buildingTags'=>$buildingTags,
	...
	public function actionBuildList() {
        ...
        $get = SearchMenu::explodeAllParamsToArray();
		//print_r($get);
        $dataProvider = self::getDataProvider($get);//默认条件:上海城市的租房信息
        ...
        $this->render('buildlist',array(
                ...
                'options'=>$get,//生成的数组。在前台生成连接时使用
                'seotkd'=>$seotkd,//SEO优化
                'url'=>"systembuildinginfo/buildlist",//url
        ));
