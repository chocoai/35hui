...
class SiteController extends Controller {
    ...
    public function actionIndex() {
        ...
        if($this->beginCache("siteindex_data",array('duration'=>Yii::app()->params['duration']))) {
            ...
            $criteria->condition="fid=41 and displayorder=0 and moderated=0";
            $bbsZX=Forum_thread::model()->findAll($criteria);
            
            $officeCriteria=new CDBCriteria;
            $officeCriteria->limit=6;
            $officeCriteria->order="ob_updatedate desc";
            $officeCriteria->select="ob_officeid,ob_sysid,ob_officearea,ob_sellorrent,ob_avgprice,ob_rentprice,ob_titlepicurl,ob_sumprice";
            $officeCriteria->condition="ob_sellorrent =1 and ob_check =4";
            $officeRent=Officebaseinfo::model()->findAll($officeCriteria);
            $officeCriteria->condition="ob_sellorrent =2 and ob_check =4";
            $officeSale=Officebaseinfo::model()->findAll($officeCriteria);

            $officeCriteria=new CDBCriteria;
            $officeCriteria->order="rand()";
            $officeCriteria->limit="5";
            $officeCriteria->condition="ob_titlepicurl!='' and ob_check =4 and ob_releasedate >".(time()-86400*90);
            $interestOffice=Officebaseinfo::model()->findAll($officeCriteria);
            
            $systembuild=Homelist::model()->find("hl_type=1");
            $creativeparkbaseinfo=Homelist::model()->find("hl_type=2");
            $businesscenter=Homelist::model()->find("hl_type=3");
            ...
            $this->render('index',array(
                    ...
                    "interestOffice"=>$interestOffice,
                    "systembuild"=>$systembuild,
                    "creativepark"=>$creativeparkbaseinfo,
    ...
    public function actionRegister() {
        ...
        header("Location:/site/agentregister");
...
//    }
    ...
    public function actionOfficeHeadsearch() {
        if(isset($_POST)) {//只接受post传值
            ...
            $type = @$_POST['type'];//类型1写字楼2商务中心3经纪人搜索房源4经纪人搜索创意园区
            ...
            if($type==1) {//写字楼
                ...
                $url = @$_POST["url"];
                $this->redirect(array($url,"search"=>'keyword'.$keyword));
            }elseif($type==3) {//经纪人搜索房源
                $action = $sellorrent==1?"officerent":"officesale";
                $this->redirect(array('uagent/'.$action,"search"=>'keyword'.$keyword));
            }elseif($type==4) {//经纪人搜索创意园区
                $this->redirect(array('uagent/creativesource',"search"=>'keyword'.$keyword));
            }else {
    ...
    public function actionShopHeadsearch() {
        if(isset($_POST)) {//只接受post传值
            ...
            if($type==1) {//商铺
                ...
                $this->redirect(array('uagent/shoprent',"search"=>'keyword'.$keyword));
            }if($type==4) {//经纪人商铺出售页
                $this->redirect(array('uagent/shopsale',"search"=>'keyword'.$keyword));
            }if($type==5) {//经纪人商铺转让页
                $this->redirect(array('uagent/shoptransfer',"search"=>'keyword'.$keyword));
            }
        }else {
            $this->redirect(array('site/index'));
        }
    ...
    */
    public function actionIndexSearch() {
        ...
        }elseif($_GET['type']==7) {//商铺转让
            $this->redirect(array('shop/transferIndex',"search"=>$param));
        }else {//小区
            $this->redirect(array('site/index'));
        }
        $this->redirect(array('site/index'));
