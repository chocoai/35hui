...
class ThumbHandler
{
     ...
     */
    function setDstImg($dst_img,$ifMkdir=true){
        if($ifMkdir){
            $arr  = explode('/',$dst_img);
            $last = array_pop($arr);
            $path = implode('/',$arr);
    ...
    function setImgCreateQuality($n){
        $this->img_create_quality =intval($n);
    }
    /**
     * 设置文字水印
     *
  ...
  private   function _output(){
        ...
        if(function_exists($func_name)){
            ...
            //         
            if($func_name($this->h_dst, $this->dst_img, $this->img_display_quality)){
                return true;
            }else{
                return false;
            }
  ...
  private   function _setNewImgSize($img_w, $img_h=null){
        ...
        if(2 == $num){
            ...
            switch($this->cut_type){
                case 0: // 如果原图大于缩略图，产生缩小，否则不缩小
                    ...
                    $this->dst_h   = $this->fill_h + $this->img_border_size*2;
                    break;
                case 1: // 自动裁切  // 如果图片是缩小剪切才进行操作  // 目标尺寸
                    if($rate_w >= 1 && $rate_h >=1){
   ...
   private function _flipV($src){
     ...
     * @return unknown_type
     */
    function dispalyImg($fileName='',$string='',$deleteFlag=false){
