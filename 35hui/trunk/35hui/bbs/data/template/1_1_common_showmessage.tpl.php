...
<p class="alert_btnleft"><a href="<?php echo $url_forward;?>">如果你的浏览器没有自动跳转，请点击此链接</a></p>
<?php } else { ?>
...
<h3 class="flb"><em>提示信息</em><?php if($_G['inajax']) { ?><span><a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');" title="关闭">关闭</a></span><?php } ?></h3>
<div class="c altw">
...
<?php } if($param['login']) { ?>
<button type="button" class="pn pnc" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');showWindow('login', 'member.php?mod=logging&action=login');"><strong>登录</strong></button>
<?php if(!$_G['setting']['bbclosed']) { ?>
<button type="button" class="pn pnc" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');window.open('member.php?mod=<?php echo $_G['setting']['regname'];?>');"><em><?php echo $_G['setting']['reglinkname'];?></em></button>
<?php } ?>
<button type="button" class="pn" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');"><em>取消</em></button>
<?php } else { ?>
<button type="button" class="pn pnc" id="closebtn" onclick="hideWindow('<?php echo $_G['gp_handlekey'];?>');"><strong>确定</strong></button>
