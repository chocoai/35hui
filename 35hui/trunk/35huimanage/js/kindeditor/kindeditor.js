...
KE.util = {
	...
	getData : function(id, wyswygMode) {
		...
		wyswygMode = (wyswygMode === undefined) ? g.wyswygMode : wyswygMode;
                if(g.iframeDoc.body.innerHTML.length > g.maxLength+50){
                    alert(KE.lang.txtOverError.replace('[maxlength]',g.maxLength));
                    g.iframeDoc.body.innerHTML=g.iframeDoc.body.innerHTML.substring(0,g.maxLength);
                }
		if (!wyswygMode) {
...
KE.lang = {
	...
	pasteError : '您的浏览器安全设置不允许使用粘贴操作，请使用快捷键(Ctrl+V)来完成。',
        txtOverError : '您的输入超过了设定的[maxlength]文本长度,请减少输入量。'
};
