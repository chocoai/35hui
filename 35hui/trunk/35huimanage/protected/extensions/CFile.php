...
//			throw new CHttpException(500, 'Unable to get directory content for "'.$directory.DIRECTORY_SEPARATOR.'"');
	 ...
	 * to be copied to
     * @param string $includeCurrentFolder 如果指定路径是一个文件夹,是否将该文件夹一起拷过去
	 * @return mixed New CFile object for newly created filesystem object on
	 * success, 'false' on fail.
	 */
	public function copy($fileDest,$includeCurrentFolder=false)
	{
		...
		else
		{
            if($includeCurrentFolder){
                $currentFolder = $this->getBasename();
                $destRealPath = $this->resolveDestPath($fileDest.DIRECTORY_SEPARATOR.$currentFolder);
                $this->createDir(0754, $destRealPath);
            }
	...
	/**
     * 转移文件或者文件夹里的所有内容
     * @param <string> $fileDest 新的路径
     * @param <boolean> $includeFolder 如果当前路径是一个文件夹,是否连当前文件夹一起转移
     * @return <boolean> 转移成功或失败
     */
	public function move($fileDest,$includeFolder=false)
	{
        $oldInstance = $this;//旧的路径
        $destRealPath = $this->resolveDestPath($fileDest);
        $copyInstance = $this->copy($fileDest,$includeFolder);
        if($copyInstance){
            return $oldInstance->delete();
        }
        return false;
	}
