...
$this->currentMenu = 38;
?>
<table width="100%" style="margin: 0px" border="1">
    <tr>
        <th width="40%">标题</th>
        <th width="10%">用户Id</th>
        <th width="8%">用户</th>
...
<div style="display:none" id="showTip">
    <form action="#" id="auditform">
        ...
        <div>
            <input type="hidden" name="auditshopid" id="auditshopid" />
            是否优质：<input type="checkbox" name="st_ishigh" id="st_ishigh" onclick="showPoint(this)"/>是<br />
...
<div style="display:none" id="showTip2">
    <form action="#" id="illegalreason">
        <div style="float:right"><img onClick="closetip()" src="<?php echo IMAGE_URL."/3.gif";?>"/></div>
        <div>
            <input type="hidden" name="auditshopid" id="illegalshopid" />
...
function pass(){
    ...
    $.ajax({
        url:"<?php echo Yii::app()->createUrl("/shopbaseinfo/audit") ?>",
        type:"GET",
        data:data+"&type=pass",
        success:function(){
