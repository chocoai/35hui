...
class Fundsconfig extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('fc_rmbprice, fc_giveprice, fc_givepoint, fc_type, fc_givepanoramadevice,fc_vipexp', 'numerical', 'integerOnly'=>true),
	...
	public function search()
	{
        ...
        public function formatData(){
            $dba = dba();
            $fcdata = $dba->select("select * from `35_fundsconfig` where `fc_type`=1");
            $result = array();
            foreach($fcdata as $data){
                $value='RMB'.$data['fc_rmbprice'].'&nbsp;&nbsp;商务币'.$data['fc_giveprice'].'&nbsp;&nbsp;积分'.$data['fc_givepoint'];
                if($data['fc_givepanoramadevice']){
                    $value.= '&nbsp;&nbsp;全景镜头'.$data['fc_givepanoramadevice'].'枚';
                }
                $result[$data['fc_id']] = $value;
            }
            return $result;
        }
}