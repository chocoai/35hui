 ...
 * add functions by skding @ 2011.06.23
 *  textMark($src,$markText)
 *  outImage( & $img,$fileName)
 */

class Image {
    public $fontSize = 8;
    public $fontFile = 'protected/config/arial.ttf';
    public $fontColor = '#FFFFFF';//文字颜色
    public $fontAlpha = 95;//0~127 0不透明
    public $markTimes = 1;//水印次数 >= 1
    /** 类保护变量 */
    protected $th_width    = 200;
    protected $th_height   = 75;
    protected $quality     = 90;   //图片质量
     ...
     * @param strubg $markText 水印文字
     */
    public function textMark($src,$markText) {
        //计算水印文本的大小
        $img = $this->IM($src);
        $box = imagettfbbox($this->fontSize,0,$this->fontFile,$markText);
        //echo $box[2]-$box[6],'_',$box[3]-$box[7];
        $mark_x = imagesx($img) - $box[2]-$box[6];
        $mark_y = imagesy($img) - $box[3]-$box[7];
        //将文字打印到图片上
        $RGB=$this->colorHexRgb($this->fontColor);
        //$imageColor=imagecolorallocate($img,$RGB[0],$RGB[1],$RGB[2]);
        $imageColor = imagecolorallocatealpha($img,$RGB[0],$RGB[1],$RGB[2], $this->fontAlpha);
        $markTimes = $this->markTimes;
        do{
            imagettftext($img,$this->fontSize,0,rand(0,$mark_x),rand(0,$mark_y),$imageColor,$this->fontFile,$markText);
        }while(--$markTimes);
        $this->outImage($img, $src);
        imagedestroy($img);
    }
    /**
     * 返回颜色的RGB值 eg #00FFCC
    ...
    public function formatWithPicture($src,$formatArray,$ifMark=false,$changeDefaultSize=true, $marksrc = '/images/mark.png'){
        ...
        foreach($formatArray as $formatStand) {
            ...
            imagecopy($capture, $src_im, 0, 0, $bg_x, $bg_y, $formatStand['width'], $formatStand['height']);
     ...
     * @return <boolean> 是否生成成功
     */
//    public function formatWithPicture($src,$formatArray,$ifMark=false,$marksrc = '/images/mark.png') {
//        $successNum = 0;
//        if($formatArray) {
...
//                }
////                $thumbimg = $this->capture($src, $newImgName, $formatArray);
...
//                    $this->mark($newImgName,$_SERVER['DOCUMENT_ROOT'].$marksrc,$newImgName);//$newImgName
//                    $this->mark($src,$_SERVER['DOCUMENT_ROOT'].$marksrc,$src);//$newImgName
...
//    }
    ...
    public function mark($src, $mark, $dst='', $output=true) {
        ...
        $px = $sw - $mw;
        $py = $sh - $mh;
        $cx=($sw-$mw*(floor($sw/$mw)))/2;
        $cy=($sh-$mh*(floor($sh/$mh)))/2;
        ...
        if(strpos($mark, 'mark2.jpg') !== FALSE){
           $this->transparent = 100;
        }
        if(preg_match('/\.png$/i', $mark)){
           // $this->alpha_blending($im,$mim,$px,$py);
            for($ix=$cx;$ix<=$px;$ix+=$mw){
                for($iy=$cy;$iy<=$py;$iy+=$mh){
                    $this->alpha_blending($im,$mim,$ix,$iy);
                }
            }
        }else{
            for($ix=0,$iy=0;$ix<$sw - $mw-5,$iy<$sh - $mh-5;$ix+=$mw,$iy+=$mh){
                imagecopymerge($im, $mim, $ix, $iy, 0, 0, $mw, $mh, $this->transparent);
    ...
    /**
     * 第一房屋全景增加水印
     * @param <type> $src 要加水印的图片
     * @param <type> $mark 水印图片
     * @param <type> $dst 保存地址
     */
    public function panoMask($src, $mark, $dst=''){
        $result = false;
        if(!file_exists($src) || !file_exists($mark)) {
            $this->error="File not exists.";
            return $result;
