<ul onmouseout="this.className='threeline_boxleftulthree  addserach'" onmouseover="this.className='threeline_boxleftulfour  addserach'" class="threeline_boxleftulthree  addserach">
    ...
    <li class="ulthreetwo">
        ...
        <span style="color: black;"><?=Officebaseinfo::model()->getFitment($data->sb_adrondegree)?>&nbsp;&nbsp;楼层：<?=$data->sb_floor."/".$data->sb_allfloor?>&nbsp;月租：<?=$data->rentInfo['sr_monthrentprice']?'<font color="red">'.$data->rentInfo['sr_monthrentprice'].'</font>元/月':"暂无资料"?> </span><br>
    ...
    <li class="ulthreethree"><span class="show_price"><?=CHtml::encode($data->rentInfo['sr_rentprice'])?></span><br>
        <span style="color: black;">元/平米·天</span>
    </li>
</ul>